...
class HypersphericalUniform(distributions.Distribution):
    ...
    def __init__(self, dim, dtype=dtypes.float32, validate_args=False,
                 allow_nan_stats=True, name="HypersphericalUniform"):
    ...
    def _log_prob(self, x):
        return - array_ops.ones(shape=array_ops.shape(x)[:-1],
                                dtype=self.dtype) * self.__log_surface_area()
